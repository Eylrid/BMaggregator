...
Info about BMaggregator can be found at bittext.ch/btDiTl8V_d
''' %(startTimeString, endTimeString)
        ...
        report += u'\nBroadcasts:\n-----------\n'
        subscriptions = self.getMessageCounts(self.getSubscriptions(), startTime, endTime)
        subscriptions.sort(reverse = True)

        for count, address, label in subscriptions:
            report += u'%d\t%s\t%s\n' %(count, address, label)

        #subjects = []
        #for subject in self.subjects:
        #    messages = [msg for msg in self.subjects[subject]
        #                if int(msg.receivedTime) >= startTime
        #                and int(msg.receivedTime) <= endTime]
        #    if messages:
        #        subjects.append((len(messages), subject))

        #report += '\nSubjects:\n---------\n'
        #subjects.sort(key = lambda x: x[1])
        #subjects.sort(key = lambda x: x[0], reverse=True)

        #for count, subject in subjects:
        #    report += u'%d\t%s\n' %(count, subject)
